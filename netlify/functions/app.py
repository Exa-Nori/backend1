...
def handle_telegram_errors(func):
    ...
    def wrapper(event, context):
        try:
            ...
            return create_response(500, {"error": "Ошибка сети при обращении к Telegram API"})
        except Exception as e:
            return create_response(500, {"error": "Внутренняя ошибка сервера"})
    return wrapper
...
def handler(event, context):
    ...
    if path == '/api/test-telegram' and method == 'GET':
        ...
        return create_response(404, {"error": "Endpoint не найден"}) 